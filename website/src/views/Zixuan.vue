  ...
  <div class="py-8 px-4 sm:px-6 lg:px-8 w-full">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
      <div>
        ...
        <p class="mt-2 text-gray-500">全面分析您的持仓股票，实时追踪市场动态</p>
      </div>
...
const { getStocks, loading } = useStockApi()
const saving = ref(false)

const stocksData = ref<PortfolioItem[]>([])
...
const fetchData = async () => {
  try {
    ...
    updateFilterOptions()
    console.log('数据加载完成，stocksData长度:', stocksData.value.length)
  } catch (error) {
...
const saveData = async () => {
  ...
  try {
    // 获取最新的持仓数据（带实时价格/MA）
    const latest = await getStocks()
    const res = await fetch('/api/save-stocks', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ stocks: latest })
    })
...
const exportExcel = () => {
  ...
  try {
    ...
    // 添加工作表到工作簿
    XLSX.utils.book_append_sheet(wb, ws, '持仓分析')
    ...
    const fileName = `自选分析${filterStr}_${dateStr}_${timeStr}.xlsx`
